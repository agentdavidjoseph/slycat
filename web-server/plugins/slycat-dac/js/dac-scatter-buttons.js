...
import scatter_plot from "./dac-scatter-points";
...
module.setup = function (sel_color, init_subset_flag, init_fisher_order, 
                         init_fisher_pos, init_diff_desired_state,
                         VAR_INCLUDE_COLUMNS, datapoints_meta, meta_include_columns,
	...
	this.change(function()
		{
            ...
            // set up color plot
            compute_color_col(select_col);
            //scatter_plot.color_draw(curr_color_by_col);
...
function update_color_by_col (select_col)
{
    ...
    // actual meta data coloring
        ...
        $.when(request.get_table("dac-datapoints-meta", mid)).then(
            function (data)
            {
                compute_color_col_data (data, select_col);
            },
            function ()
            {
